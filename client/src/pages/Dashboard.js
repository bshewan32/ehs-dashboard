...
import { fetchReports, fetchMetricsForPeriod } from '../components/services/api';
...
export default function Dashboard() {
  ...
  // Function to trigger KPI refresh
  const triggerKPIRefresh = useCallback(() => {
    console.log('Triggering KPI refresh');
  ...
  const handlePeriodChange = useCallback((periodId, periodValue) => {
    ...
    setLastFetchTime(0);
    // Also trigger KPI refresh when period changes
    triggerKPIRefresh();
  }, [triggerKPIRefresh]);
  ...
  const handleCompanyChange = useCallback((company) => {
    ...
    setLastFetchTime(0);
    // Also trigger KPI refresh when company changes
    triggerKPIRefresh();
  }, [triggerKPIRefresh]);
  ...
  // Listen for KPI changes from localStorage events (when KPIs are updated in other tabs/windows)
  useEffect(() => {
    const handleStorageChange = (e) => {
      if (e.key === 'kpiData') {
        console.log('KPI data changed in localStorage, triggering refresh');
        triggerKPIRefresh();
      }
    };
